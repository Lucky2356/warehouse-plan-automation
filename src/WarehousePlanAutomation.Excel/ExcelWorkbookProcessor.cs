...
public sealed class ExcelWorkbookProcessor : IWorkbookProcessor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var outputPath = OutputPath.Build(sourceFilePath, _nowProvider(), OutputFileName.PlanMark);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Названия листов читаются до закрытия книги: после Close обращение к листу
            // уже не проходит, а замечаниям они нужны, чтобы по ним можно было перейти.
            var warnings = BuildWarnings(
                classification,
                update,
                ExcelSheetOperations.GetSheetName(sheets.Orders),
                ExcelSheetOperations.GetSheetName(sheets.Plan));
            ...
            return new ProcessingResult(
                ...
                new[]
                {
                    new ProcessingCounter("обработано строк", classification.RowsToDelete.Count),
                    new ProcessingCounter(
                        "осталось вручную",
                        classification.Leftovers.Count,
                        classification.Leftovers.Count > 0),
                    new ProcessingCounter("новых заказов", update.NewRows.Count),
                    new ProcessingCounter("удалено заглушек", update.PlanRowsToDelete.Count),
                },
                warnings);
        }
        finally
    ...
    /// <summary>
    /// Замечания, которые программа не стала решать за человека: строки выгрузки,
    /// не подошедшие ни под одно правило, и заказы, пропавшие из сегодняшней выгрузки.
    /// </summary>
    private static IReadOnlyList<ProcessingWarning> BuildWarnings(
        OrderClassificationResult classification,
        PlanStructuralUpdate update,
        string ordersSheetName,
        string planSheetName)
    {
        var warnings = new List<ProcessingWarning>();

        foreach (var row in classification.Leftovers)
        {
            warnings.Add(new ProcessingWarning(
                "Строка не подошла ни под одно правило, разберите вручную: " + row.Comment,
                SheetSchema.OrdersSheet + ", строка " + row.ExcelRow,
                ordersSheetName,
                "A" + row.ExcelRow));
        }

        foreach (var order in update.OrderUpdates.Where(u => u.MissingFromOrders))
        {
            warnings.Add(new ProcessingWarning(
                "Заказ " + order.LoadNumber + " пропал из выгрузки: количество обнулено, " +
                "номер загрузки подсвечен розовым.",
        ...
        ColumnRange columns)
    {
        foreach (var spec in specs)
        {
            ...
            RestoreCalculatedFormulas(planSheet, insertBefore, spec);
            _logger.Debug("Добавлена строка заказа " + spec.LoadNumber + " в строку " + insertBefore + ".");
        }
    }

    /// <summary>Колонки, которые у строк заказа считаются формулами.</summary>
    private static readonly string[] CalculatedColumns =
    {
        SheetSchema.Plan.DaysInWork,
        SheetSchema.Plan.NetworkDate,
        SheetSchema.Plan.ShipmentDate,
        SheetSchema.Plan.Decision,
    };

    /// <summary>
    /// Новая строка считает «Дней в работе», «Дату в сети», «Дату отгрузки» и «Решение»
    /// так же, как остальные строки плана. Если у образца в какой-то из этих колонок
    /// формулы не было (значение вписали руками), формула берётся у ближайшей строки,
    /// где она есть, - сначала в том же блоке, потом в любом блоке заказов - и переносится
    /// в относительной записи, как при протягивании. «Дата в сети», взятая из текста
    /// поставки, остаётся значением.
    /// </summary>
    private void RestoreCalculatedFormulas(object planSheet, int row, NewPlanRowSpec spec)
    {
        var layout = ReadPlanLayout(planSheet);
        var added = layout.AllDataRows.FirstOrDefault(r => r.ExcelRow == row);
        if (added is null)
        {
            return;
        }

        foreach (var name in CalculatedColumns)
        {
            if (added.FormulaColumns.Contains(name) ||
                (name == SheetSchema.Plan.NetworkDate && spec.NetworkDate is not null) ||
                !layout.Headers.TryGet(name, out var column))
            {
                continue;
            }

            var donor = layout.OrderSections
                .SelectMany(s => s.DataRows)
                .Where(r => r.IsOrderRow && r.ExcelRow != row && r.FormulaColumns.Contains(name))
                .OrderBy(r => r.Section == added.Section ? 0 : 1)
                .ThenBy(r => Math.Abs(r.ExcelRow - row))
                .FirstOrDefault();

            var formula = donor is null
                ? null
                : ExcelSheetOperations.GetFormulaR1C1(planSheet, donor.ExcelRow, column);

            if (formula is null || !formula.StartsWith("=", StringComparison.Ordinal))
            {
                _logger.Warning(
                    "В строке " + row + " не заполнена «" + name + "»: на листе нет строки заказа с формулой в этой колонке.");
                continue;
            }

            ExcelSheetOperations.SetFormulaR1C1(planSheet, row, column, formula);
            _logger.Debug("«" + name + "» строки " + row + " - формула строки " + donor!.ExcelRow + ".");
        }

        AdjustUrgentNetworkDate(planSheet, layout, row, spec);
    }

    /// <summary>
    /// У срочной подтоварки «Дата в сети» на 4 дня раньше: «=F5+$O$2-4». Формула новой строки
    /// взята у соседней, поэтому вычитание в ней приводится к тексту самой строки.
    /// </summary>
    private void AdjustUrgentNetworkDate(object planSheet, PlanLayout layout, int row, NewPlanRowSpec spec)
    {
        if (spec.NetworkDate is not null || !layout.Headers.TryGet(SheetSchema.Plan.NetworkDate, out var column))
        {
            return;
        }

        var formula = ExcelSheetOperations.GetFormulaR1C1(planSheet, row, column);
        if (formula is null || !formula.StartsWith("=", StringComparison.Ordinal))
        {
            return;
        }

        var urgent = OrderTextRules.IsUrgentRestock(spec.Supplies);
        var adjusted = OrderTextRules.NetworkDateFormula(formula, urgent);
        if (!string.Equals(adjusted, formula, StringComparison.Ordinal))
        {
            ExcelSheetOperations.SetFormulaR1C1(planSheet, row, column, adjusted);
            _logger.Debug("«Дата в сети» строки " + row + (urgent ? ": срочная подтоварка, минус 4 дня." : ": без вычитания срочной подтоварки."));
        }
    }

    /// <summary>Красный цвет даты в тексте поставки (RGB 255 0 0, в порядке BGR).</summary>
    private const int NetworkDateTextColor = 0x0000FF;
    ...
    private static void FillNewRow(object planSheet, HeaderMap headers, int row, NewPlanRowSpec spec)
    {
        ...
        foreach (var pair in headers.Columns)
        {
            ...
            // Дата в сети из текста поставки встаёт значением и поверх формулы образца:
            // считать её от даты документа для такой строки неверно.
            if (pair.Key == SheetSchema.Plan.NetworkDate && spec.NetworkDate is { } networkDate)
            {
                ExcelSheetOperations.SetValue(planSheet, row, column, networkDate.OADate);
                continue;
            }
            ...
            switch (pair.Key)
            {
                case SheetSchema.Plan.Supplies:
                    ...
                    if (spec.NetworkDate is { } date)
                    {
                        ExcelSheetOperations.SetTextColor(
                            planSheet, row, column, date.Start, date.Length, NetworkDateTextColor);
                    }
