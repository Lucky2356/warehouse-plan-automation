...
public static class PlanUpdateBuilder
{
        ...
        DateTime today)
    {
        ...
        var (newRows, plannedMatches) = BuildNewRows(plan, orders.Groups, yesterdayLoadNumbers, today);
        ...
        DateTime today)
    {
        ...
        foreach (var group in groups)
        {
            ...
            var documentDate = NormalizeDocumentDate(group.DocumentDate);
            result.Add(new NewPlanRowSpec(
                ChooseSection(group.Comment),
                supplies,
                processing,
                OrderTextRules.LoadedComment,
                documentDate,
                group.LoadNumber,
                NetworkDateParser.Find(supplies, documentDate, today)));
        }

        // Новые строки встают внизу блока подряд - в том порядке, какой задан здесь.
        return (NewPlanRowOrder.Sort(result), matches);
    }

    /// <summary>
    /// Блок новой строки. «Приемка» проверяется раньше «возврата»: заказ
    /// «…_получение в рознице 01.10_приемка на хранилище из возвратов» стоит в плане
    /// в блоке «приемка на хранилище», хотя слово «возвратов» в нём тоже есть.
    /// </summary>
    private static PlanSectionKind ChooseSection(string comment)
    {
        if (OrderTextRules.IsStorageAcceptance(comment))
        {
            return PlanSectionKind.StorageAcceptance;
        }

        return OrderTextRules.IsReturn(comment) ? PlanSectionKind.Returns : PlanSectionKind.AllGroups;
    }
