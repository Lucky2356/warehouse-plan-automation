...
public sealed class PlanPriorityComparer : IComparer<PlanSortItem>
{
    ...
    /// взаимный порядок. Дата им не придумывается: это работа, не привязанная к дате
    /// появления в рознице («отгрузка по готовности», «приемка на хранилище_N приоритет»),
    /// и во вчерашнем плане она стоит в начале блока. Отправлять такие строки вниз
    /// значило бы считать их дату бесконечно поздней, то есть придумать значение.
    /// </summary>
