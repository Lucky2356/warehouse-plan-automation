...
public class OrderClassifierTests
{
    ...
    public void Опт_СуммируетсяИСтрокиУдаляются()
    {
        var rows = new[]
        {
            ...
            Row(4, "Москва-M614", "Подтоварка Номер загрузки 44575395", 10),
        };
    ...
    [Theory]
    [InlineData("Магнитогорск-М66")]
    [InlineData("Магнитогорск-М62")]
    public void Магнитогорск_НеСчитаетсяМаркетплейсом(string division)
    {
        ...
        var rows = new[]
        {
            Row(2, division, "Подтоварка ШПП Номер загрузки 44575395 <Подбор:>", 40),
        };
        ...
        Assert.Equal(44575395L, group.LoadNumber);
    ...
    public void Маркетплейсы_ВозвратыОбрабатываютсяРаньшеПоставок()
    {
        ...
        var rows = new[]
        {
            Row(2, "Ozon", "Ozon Мск Крупный микс из Возвратов и С326-058 приоритет к 06.08", 40),
        };
    ...
    public void Маркетплейсы_РазделяютсяНаВозвратыПоставкиИХранение()
    {
        var rows = new[]
        {
            ...
            Row(3, "Ozon", "Озон Екб МЗ2446-002, МЗ798-151 (Шапки) РФ", 20),
    ...
    [Theory]
    [InlineData("Заказ на магазин 001 из заказов МП автозаказ Номер загрузки 44386696")]
    [InlineData("СЗ798-154(шапки), почта Росс, виртуально, 1 место Номер загрузки 44050468")]
    ...
    [InlineData("2448-008 в ОФис на ремонт с возвратом.")]
    [InlineData("В Офис 2448-008 Ремонт с возвратом WB")]
    [InlineData("Укомплектовать в наборы МЗ313-027 (палантин+брошь). Списать со склада.")]
    [InlineData("Маркировка.")]
    [InlineData("Виртуальный возврат")]
    public void СлужебныеСтроки_УдаляютсяБезПереноса(string comment)
    {
        var rows = new[] { Row(2, "Новосибирск-M67a", comment, 12) };
    ...
    [InlineData("ЗП373-1739 Номер загрузки 44388195 <Подбор:>")]
    ...
    public void СтрокиЗП_НеВлияютНаИтогиПодразделений()
    {
        ...
        var rows = new[]
        {
            Row(2, "Опт Москва", "ЗП373-1739 отгрузка", 40),
            Row(3, "Ozon", "ЗП373-1740 Озон Екб", 25),
        };
    ...
    public void РеальныеЗаказы_ГруппируютсяПоНомеруЗагрузки()
    {
        var rows = new[]
        {
            Row(2, "Москва-M614", "Срочная подтоварка 28.08_Хранение Номер загрузки 44575395 <Подбор:>", 100),
            Row(3, "Иркутск-М57", "Срочная подтоварка 28.08_Хранение Номер загрузки 44575395 <Подбор:>", 50),
            Row(4, "Казань-М613", "2412-015 Пуховики Номер загрузки 44231895 <Подбор:>", 7),
        };
        ...
        Assert.Equal(44575395L, first.LoadNumber);
    ...
    public void СтрокиБезНомераЗагрузки_ОстаютсяДляРучнойПроверки()
    {
        ...
        var rows = new[]
        {
            ...
            Row(3, "Москва-M614", "Подтоварка Номер загрузки 44575395", 8),
        };
