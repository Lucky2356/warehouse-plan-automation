...
public class SheetReaderTests
{
    ...
    public void ВыгрузкаЗаказов_КолонкаРазницаЕдНаходитсяПоНачалуЗаголовка()
    {
        var grid = SheetGrid.FromRows(1, 1, new List<object?[]>
        {
            ...
            new object?[]
            {
                46210.7d, 46210d, "---", "З000-754935", "Ozon",
    ...
    public void Журнал_КолонкаПроцентаНаходитсяТолькоПоТочномуСовпадению()
    {
        var grid = SheetGrid.FromRows(1, 1, new List<object?[]>
        {
            ...
            new object?[]
            {
                "Магазины M", "ЗАКРЫТ", "Подтоварка Номер загрузки 44575395", 0d, "-17", "-32", "-32",
    ...
    /// <summary>Строка заголовков подтоварки - как в готовой книге аналитика.</summary>
    private static SheetGrid LoadSheetHeaders(string comment, string quantity = "в подтоварку", string? extra = null) =>
        SheetGrid.FromRows(1, 1, new List<object?[]>
        {
            new object?[]
            {
                "Код клиента", "Сектор", "Группа", "АЦР", "Артикул", "Цвет", "Размер", "Код", "РЦ",
                quantity, comment, "Приоритет",
                "Фактическое кол-во, которое можно собрать на ВБ+Озон", "Комментарий (на запрет)",
                "Прогнозный sellout", "Заметки", extra,
            },
        });

    [Theory]
    [InlineData("в подтоварку")]
    [InlineData("в подтоварку Мск")]
    [InlineData("В подтоварку МСК")]
    [InlineData("в подтоварку Екб")]
    public void Подтоварка_КолонкаВПодтоваркуМожетБытьПодписанаСкладом(string quantity)
    {
        var headers = RestockSchema.Load.ResolveHeaders(LoadSheetHeaders("комент", quantity));

        Assert.Equal(10, headers[RestockSchema.Load.Quantity]);
    }

    [Fact]
    public void Подтоварка_ЗаголовокСПодтоваркойВСерединеНеСчитается()
    {
        Assert.Throws<WorkbookValidationException>(() =>
            RestockSchema.Load.ResolveHeaders(LoadSheetHeaders("комент", "урезать в подтоварку")));
    }
    ...
    public void Подтоварка_ЗапретИКоментРазбираютсяПоРазнымКолонкам(string comment)
    {
        var headers = HeaderResolver.Resolve(
            LoadSheetHeaders(comment), RestockSchema.LoadSheet, RestockSchema.Load.Specs);

        Assert.Equal(14, headers[RestockSchema.Load.Ban]);
        Assert.Equal(11, headers[RestockSchema.Load.Comment]);
        Assert.Equal(13, headers[RestockSchema.Load.Available]);
        Assert.Equal(16, headers[RestockSchema.Load.Note]);
    }

    [Fact]
    public void Подтоварка_БезКолонкиКомента_ЖалуетсяНаНеёЖе()
    {
        // У «Комментарий (на запрет)» при поиске отбрасывается скобка, и она становится
        // «комментарием» - псевдонимом «комента». «Запрет» ищется раньше и забирает её
        // себе, иначе программа сказала бы, что не нашла «Запрет», хотя он на листе есть.
        var error = Assert.Throws<WorkbookValidationException>(() => HeaderResolver.Resolve(
            LoadSheetHeaders("Место хранения"), RestockSchema.LoadSheet, RestockSchema.Load.Specs));

        Assert.Contains("комент", Assert.Single(error.Problems));
